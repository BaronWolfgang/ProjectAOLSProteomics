...
compounds_with_MoA %>% pull() %>% table() 

compounds_with_MoA_simp <- compounds_with_MoA %>% 
  mutate(compound_simp = case_when(
    compound == "Small molecule" ~ "Other",
    compound == "Hormone-like" ~ "Other",
    compound == "Cytp450" ~ "Other",
    compound == "Calcineurin inhibitor" ~ "Other",
    TRUE ~ compound
  ))

length(compounds_with_MoA_simp %>% unique())

paper_compound_types <- c()
```
...
comparison_data_summary %>% 
  ...
  geom_point(aes(y=-8900, col=Type), size=2 ) +
...
doseresponse_norm %>%
  filter(!is.na(value), 
         is.finite(value)) %>%
  distinct(Genes, compound, dose, .keep_all = TRUE) %>%
  mutate(log2FC = log2(value / value[dose == 0.1]))
```


```{r}
doseresponse_log2FC <- doseresponse_norm %>%
  filter(!is.na(value), 
         is.finite(value)) %>%
  distinct(Genes, compound, dose, .keep_all = TRUE) %>%
  mutate(
    log2FC = log2(value / value[dose == 0.1]),
    compound = factor(compound, levels=compound_ranked_regulated)
    ) %>%
  arrange(compound) %>%
  filter(dose != 0.1)
