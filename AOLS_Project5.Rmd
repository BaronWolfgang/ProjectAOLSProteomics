...
selected_genes <- c("HDAC1", "RRM1", "BAG3", "DNMT1", "DHFR")
...
selected_genes <- doseresponse_norm %>% select(Genes) %>% unique() %>% 
  filter(str_detect(Genes, "CLK")) %>% #No CLK3 or CLK4 in the file?? 
  arrange(Genes) %>% pull()
...
comparison_data <- doseresponse_norm %>%
  filter(dose %in% c(1, 10000), # we want to compare highest vs lowest (baseline) conditions
